...
class SBuild(implicit _project: Project) {
  ...
  val jar = s"target/org.sbuild.ant-${SBuildConfig.sbuildVersion}.jar"
  val sourcesJar = s"target/org.sbuild.ant-${SBuildConfig.sbuildVersion}-sources.jar"
  ...
  Target(jar) dependsOn "compile" ~ "LICENSE.txt" exec { ctx: TargetContext =>
    AntJar(destFile = ctx.targetFile.get, baseDir = Path("target/classes"),
      ...
      manifestEntries = Map("SBuild-ComponentName" -> "org.sbuild.ant")
    )
  }
