 ...
 *
 * The most common Ant utilities are located in [[org.sbuild.ant this]] package. Wrapped Ant tasks can be found in package [[org.sbuild.ant.tasks]] and its sub packages.
 *
 ...
 *
 * The `[[org.sbuild.ant.tasks.AntDelete]]` task is a simple wrapped Ant task, to delete files or directories.
 ...
 * }}}
 *
 * As you see, we defined a phony target with the name "clean" which has no dependencies and with will use `[[org.sbuild.ant.tasks.AntDelete]]` to delete the directory.
 * By the use of `[[org.sbuild.Path]]`, we ensure, that the required `[[java.io.File]]` is relative to the current project directory.
 *
