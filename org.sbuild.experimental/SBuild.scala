...
class SBuild(implicit _project: Project) {
  ...
  lazy val aetherJar = Target(s"target/org.sbuild.experimental.aether-${sbuildVersion}.jar") dependsOn
    "compile" ~ "LICENSE.txt" ~ aetherImplJar exec { ctx: TargetContext =>
      AntJar(
        destFile = ctx.targetFile.get,
        baseDir = Path("target/classes"),
        includes = "org/sbuild/experimental/aether/*.class",
        fileSets = Seq(
          AntFileSet(file = Path("LICENSE.txt")),
  ...
  lazy val aetherImplJar = Target(s"target/org.sbuild.experimental.aether.impl-${sbuildVersion}.jar") dependsOn
    "compile" ~ "LICENSE.txt" exec { ctx: TargetContext =>
      AntJar(
        destFile = ctx.targetFile.get,
        baseDir = Path("target/classes"),
        includes = "org/sbuild/experimental/aether/impl/*.class",
